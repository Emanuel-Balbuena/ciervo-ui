...
export function attachDismissGesture({
    ...
    function onPointerDown(event) {
        ...
        const scroller = event.target instanceof Element
            ? event.target.closest('.slt-body, .slt-card')
            : null
    ...
    /**
     * `pointercancel` NO es un soltar: es el navegador quedandose el gesto.
     *
     * Pasa en cuanto un dedo arrastra sobre la lista y el navegador decide que
     * eso era scrollear. Antes compartia handler con `pointerup`, y con una lista
     * que scrollea por dentro eso cierra el select sin que nadie lo pidiera: al
     * cancel le llegan coordenadas que no son las del gesto -- en algunos
     * navegadores (0,0) -- y `progress()` las lee como un arrastre larguisimo.
     * Aqui se deshace el arrastre, el dialogo vuelve a su sitio y no se decide
     * nada.
     */
