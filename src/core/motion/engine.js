 ...
 * @returns {{ state: Record<string, number>, animate: Function, stop: Function, getVelocity: (key: string) => number, register: Function }}
 */
export function createMotion(initial, opts = {}) {
    ...
    function getVelocity(key) {
        return channels[key]?.velocity || 0
    }

    function register(key, initialValue) {
        if (channels[key]) return
        const channel = new Channel(initialValue)
        channel.onChange = (v) => {
            state[key] = v
            onChange(key, v)
        }
    ...
    return { state, animate, stop, getVelocity, register }
}
